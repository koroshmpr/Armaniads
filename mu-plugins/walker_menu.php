...
class Custom_Walker_Nav_Menu_With_Alpine extends Walker_Nav_Menu
{
    ...
    function start_lvl(&$output, $depth = 0, $args = array())
    {
        if ($depth === 1) { // Target the second level
            ...
            $output .= '<ul :class="open ? \'lg:py-8 py-4 scale-100 opacity-100\' : \'h-0 opacity-0 scale-0\'" class="sub-menu no-underline lg:flex grid grid-cols-2 lg:gap-8 gap-5 lg:text-lg text-sm duration-500 transition-all">';
        }
    }
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if ($depth === 0) { // Target the first level
            $output .= '<li x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false" class="grid parent-menu-item justify-center">';
        } elseif ($depth === 1) { // Target the second level
            $output .= '<li class="w-fit hover:text-secondary overflow-visible">';
        } else {
            $output .= '<li>';
        }
