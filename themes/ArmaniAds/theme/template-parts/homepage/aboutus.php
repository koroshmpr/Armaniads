...
<section class="min-h-52 py-16 px-6 lg:px-0 bg-fixed" style="background-image: url('<?= $aboutus['background']['url'] ?? ''; ?>')">
	<div class="container mx-auto text-center">
		...
		<article class="py-4 text-justify"><?= $aboutus['content'] ?? ''; ?></article>
			...
			<div class="grid md:grid-cols-3 py-5 gap-6 items-center justify-center">
				<?php foreach ($cards as $card): ?>
					<div x-data="{ isHovered: false }"
						...
						<article x-transition class="transition-all text-justify z-[1] pt-5 duration-300 text-white justify-center lg:text-black ease-linear"
