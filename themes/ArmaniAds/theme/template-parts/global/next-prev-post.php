...
$imgClass = 'rounded-lg mb-2 object-cover w-32 h-24 grayscale hover:grayscale-0 transition-all ease-linear';
?>

<nav class="navigation container mx-auto post-navigation px-5 lg:px-0 my-5 pt-5 border-t border-gray-200"
	 role="navigation">
		...
		<?php if (!empty($prev_post)) { ?>
			<a class="nav-previous basis-1/3 lg:basis-1/6 flex flex-col items-center text-center"
			   ...
			   rel="prev">
				...
				<span class="text-primary font-bold hover:text-cyan-700 text-base"><?= esc_html(wp_trim_words($prev_post->post_title, 10)); ?></span>
		...
		<?php if (!empty($next_post)) { ?>
			<a class="nav-next basis-1/3 lg:basis-1/6 flex flex-col items-center text-center"
			   ...
			   rel="next">
				...
				<span
					class="text-primary font-bold hover:text-cyan-700 text-base"><?= esc_html(wp_trim_words($next_post->post_title , 10)); ?></span>
			</a>
		<?php } ?>
	</div>
</nav>
